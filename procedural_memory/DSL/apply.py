"""apply — ``DSL_REGISTRY``, the ``register`` decorator, ``apply_DSL`` dispatcher.
...
primitive (other than for those two names) under
...
# (``coloring``, ``make_grid``) reject any other value at runtime. Single
# source of truth imported by both primitive modules.
VALID_COLORS = frozenset(range(10)) | {13}
...
def apply_DSL(name, grid=None, **kwargs):
