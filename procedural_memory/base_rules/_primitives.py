...
def invert_concentric_rings(grid):
    """Reverse the color order of concentric rectangular rings.

    Reads the color at each depth (top-left corner of each ring),
    builds a reversal mapping, and applies it to all cells.
    """
    h = len(grid)
    w = len(grid[0]) if grid else 0
    max_depth = min(h, w) // 2
    ...
    # Handle odd-dimension center cell
    if min(h, w) % 2 == 1:
        center_d = min(h, w) // 2
        ring_colors.append(grid[center_d][center_d])

    n = len(ring_colors)
    mapping = {}
    for i in range(n):
        mapping[ring_colors[i]] = ring_colors[n - 1 - i]
