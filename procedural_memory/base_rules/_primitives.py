...
def and_halves_vertical(grid, result_color=2):
    """Split grid at vertical separator column, AND the two binary halves.
    Where BOTH halves have non-zero, output result_color; else 0."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    sep_c = None
    for c in range(w):
    ...
    if sep_c is None:
        return [row[:] for row in grid]
    left = [row[:sep_c] for row in grid]
    right = [row[sep_c + 1:] for row in grid]
    ...
    for r in range(h):
        for c in range(out_w):
            lv = left[r][c] if c < lw else 0
            rv = right[r][c] if c < rw else 0
            if lv != 0 and rv != 0:
                out[r][c] = result_color
    return out


def tile_3x3_hflip_alternate(grid):
    """Tile input 3x3 with alternating row-flips.
    Even tile-rows use original, odd tile-rows use horizontally-flipped version.
    Each tile-row is repeated 3 times horizontally."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    flipped = [row[::-1] for row in grid]
    out = []
    for tr in range(3):
        src = grid if tr % 2 == 0 else flipped
        for r in range(h):
            out.append(src[r] * 3)
    return out


def kronecker_self_inverted(grid):
    """Kronecker self-tile with inversion.
    Each non-zero cell in input places an inverted copy of the input
    (0/color swap) at the corresponding block position.
    Zero cells place all-zero blocks."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    out_h = h * h
    out_w = w * w
    ...
    for tr in range(h):
        for tc in range(w):
            if grid[tr][tc] != 0:
                for r in range(h):
                    for c in range(w):
                        out[tr * h + r][tc * w + c] = 0 if grid[r][c] != 0 else color
    return out


def diagonal_stripe_fill(grid):
    """Extract the repeating color cycle from non-zero anti-diagonal positions
    and fill the entire grid with diagonal stripes.  At (r,c) the output is
    color_map[(r+c) % n].  Works regardless of where the seed colors appear."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    non_zero = {}
    for r in range(h):
        for c in range(w):
            if grid[r][c] != 0:
                non_zero.setdefault(grid[r][c], []).append((r, c))
    if not non_zero:
        return [row[:] for row in grid]
    n_colors = len(non_zero)
    color_map = {}
    for r in range(h):
        for c in range(w):
            if grid[r][c] != 0:
    ...
    if len(color_map) != n_colors:
        return [row[:] for row in grid]
    out = [[0] * w for _ in range(h)]
    for r in range(h):
        for c in range(w):
            out[r][c] = color_map[(r + c) % n_colors]
    return out


def fill_rect_interior_by_size(grid, border_color=2):
    """Find rectangles bordered by border_color, fill their interior zeros
    based on the number of interior zero cells.  The fill color equals the
    count of interior zeros when that count is a valid ARC color (0-9);
    otherwise falls back to a size-rank scheme [8, 4, 3, 6, 1, 9, 7, 5]."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    out = [row[:] for row in grid]
    visited = [[False] * w for _ in range(h)]
    ...
    for r in range(h):
        for c in range(w):
            if grid[r][c] == border_color and not visited[r][c]:
                # BFS to find connected border component
                comp = []
                queue = [(r, c)]
                visited[r][c] = True
                while queue:
                    ...
                    for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                        nr, nc = cr + dr, cc + dc
                        if 0 <= nr < h and 0 <= nc < w and not visited[nr][nc] and grid[nr][nc] == border_color:
                            visited[nr][nc] = True
                            queue.append((nr, nc))
                if len(comp) < 4:
                    continue
                ...
                for ir in range(min_r + 1, max_r):
                    for ic in range(min_c + 1, max_c):
                        if grid[ir][ic] == 0:
                            interior_zeros += 1
                            interior_cells.append((ir, ic))
                if interior_zeros > 0:
                    int_side = max(bw - 2, bh - 2)
                    rects.append((interior_zeros, interior_cells, int_side))

    # Fixed mapping: interior side length -> color
    # side 3 -> 8, side 5 -> 4, side 7 -> 3, ...
    rank_colors = [8, 4, 3, 6, 1, 9, 7, 5]

    for zeros, cells, int_side in rects:
        rank = (int_side - 3) // 2
        if 0 <= rank < len(rank_colors):
            fill_color = rank_colors[rank]
        elif 0 < zeros <= 9:
            fill_color = zeros
        else:
            fill_color = rank_colors[0]
        for ir, ic in cells:
...
def fill_rect_interior_learned(grid, size_color_map, border_color=2):
    """Find rectangles bordered by border_color, fill their interior zeros
    using a learned mapping from interior-zero-count to fill color."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    out = [row[:] for row in grid]
    visited = [[False] * w for _ in range(h)]
            ...
            if grid[r][c] == border_color and not visited[r][c]:
                ...
                for ir in range(min_r + 1, max_r):
                    for ic in range(min_c + 1, max_c):
                        if grid[ir][ic] == 0:
                            interior_cells.append((ir, ic))
                n_zeros = len(interior_cells)
                fill_color = size_color_map.get(str(n_zeros))
                if fill_color is not None:
...
def diagonal_stripe_fill(grid):
    """Fill entire grid with diagonal stripes.
    Extracts 3 non-zero colors from input diagonals (r+c mod 3),
    then fills grid[r][c] = colors[(r+c) % 3]."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    # Collect non-zero pixels grouped by (r+c) % 3
    color_at = {}  # (r+c)%3 -> color
    for r in range(h):
        for c in range(w):
            v = grid[r][c]
            if v != 0:
    ...
    for r in range(h):
        for c in range(w):
            out[r][c] = color_at[(r + c) % 3]
    return out


def expand_cross_diamond(grid, bg=0):
    """Expand each 3x3 cross pattern to a 5x5 diamond pattern.
    Cross = center_color at (r,c) with arm_color at 4 cardinal neighbors.
    Expanded 5x5 template:
      C 0 A 0 C
      0 C A C 0
      A A C A A
      0 C A C 0
      C 0 A 0 C
    where C=center_color, A=arm_color."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    out = [[bg] * w for _ in range(h)]
    # Find crosses: center with 4 same-colored cardinal neighbors
    crosses = []
    for r in range(1, h - 1):
        for c in range(1, w - 1):
            center = grid[r][c]
            if center == bg:
                continue
            arm = grid[r - 1][c]
            if arm == bg or arm == center:
                continue
            if (grid[r + 1][c] == arm and grid[r][c - 1] == arm
                    and grid[r][c + 1] == arm):
                crosses.append((r, c, center, arm))
    # Stamp 5x5 diamond pattern for each cross
    template = [
        ((-2, -2), 'C'), ((-2, 0), 'A'), ((-2, 2), 'C'),
        ((-1, -1), 'C'), ((-1, 0), 'A'), ((-1, 1), 'C'),
        ((0, -2), 'A'), ((0, -1), 'A'), ((0, 0), 'C'), ((0, 1), 'A'), ((0, 2), 'A'),
        ((1, -1), 'C'), ((1, 0), 'A'), ((1, 1), 'C'),
        ((2, -2), 'C'), ((2, 0), 'A'), ((2, 2), 'C'),
    ]
    for cr, cc, center_color, arm_color in crosses:
        for (dr, dc), kind in template:
            nr, nc = cr + dr, cc + dc
            if 0 <= nr < h and 0 <= nc < w:
                out[nr][nc] = center_color if kind == 'C' else arm_color
    return out


def slide_shape_to_anchor(grid, shape_color=2, anchor_color=8, bg=0):
    """Slide shape (shape_color) toward anchor (anchor_color) until adjacent.
    The anchor stays fixed; the shape translates to touch the anchor."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    # Find bounding boxes
    shape_cells = [(r, c) for r in range(h) for c in range(w) if grid[r][c] == shape_color]
    anchor_cells = [(r, c) for r in range(h) for c in range(w) if grid[r][c] == anchor_color]
    if not shape_cells or not anchor_cells:
        return [row[:] for row in grid]
    ...
    a_min_r = min(r for r, c in anchor_cells)
    a_max_r = max(r for r, c in anchor_cells)
    a_min_c = min(c for r, c in anchor_cells)
    a_max_c = max(c for r, c in anchor_cells)
    # Determine slide direction (toward anchor on major axis)
    s_center_r = (s_min_r + s_max_r) / 2
    s_center_c = (s_min_c + s_max_c) / 2
    a_center_r = (a_min_r + a_max_r) / 2
    a_center_c = (a_min_c + a_max_c) / 2
    dr = a_center_r - s_center_r
    dc = a_center_c - s_center_c
    # Calculate offset to make shape adjacent to anchor
    if abs(dr) >= abs(dc):
        # Vertical slide
        if dr > 0:
            offset_r = a_min_r - s_max_r - 1
        else:
            offset_r = a_max_r - s_min_r + 1
        offset_c = 0
    else:
        # Horizontal slide
        offset_r = 0
        if dc > 0:
            offset_c = a_min_c - s_max_c - 1
        else:
            offset_c = a_max_c - s_min_c + 1
    # Build output
    out = [[bg] * w for _ in range(h)]
    # Place anchor
...
def fill_rects_by_interior_size(grid, bg=0, border_color=2):
    """Find rectangles drawn with border_color. Fill each rectangle's
    interior (bg cells) with a color determined by interior size.
    Size-to-color mapping is learned from the single interior 'dot'
    (a border_color pixel inside the rectangle) position relative to center."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    out = [row[:] for row in grid]
    # Find rectangles using BFS on border_color
    visited = [[False] * w for _ in range(h)]
    rects = []
    for r in range(h):
        for c in range(w):
                ...
                while queue:
                    ...
                    for dr2, dc2 in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                        ...
                        if 0 <= nr < h and 0 <= nc < w and not visited[nr][nc] and grid[nr][nc] == border_color:
                            visited[nr][nc] = True
                            queue.append((nr, nc))
                min_r2 = min(p[0] for p in component)
                max_r2 = max(p[0] for p in component)
                min_c2 = min(p[1] for p in component)
                max_c2 = max(p[1] for p in component)
                if max_r2 - min_r2 >= 2 and max_c2 - min_c2 >= 2:
                    rects.append((min_r2, min_c2, max_r2, max_c2))
    return out  # placeholder - needs color mapping


# ============================================================
# flood_fill_enclosed — fill interior 0-cells enclosed by border_color
# ============================================================

def flood_fill_enclosed(grid, border_color=3, fill_color=4, bg=0):
    """BFS from all border bg-cells to mark exterior. Remaining bg-cells are interior -> fill."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    out = [row[:] for row in grid]

    # Mark cells reachable from border via bg
    ...
    while queue:
        ...
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nr, nc = r + dr, c + dc
            if 0 <= nr < h and 0 <= nc < w and not exterior[nr][nc] and grid[nr][nc] == bg:
                exterior[nr][nc] = True
                queue.append((nr, nc))
    ...
    for r in range(h):
        for c in range(w):
            if grid[r][c] == bg and not exterior[r][c]:
                out[r][c] = fill_color
    return out


# ============================================================
# expand_cross_to_diamond — expand 3x3 cross to 5x5 diamond
# ============================================================

def expand_cross_to_diamond(grid, bg=0):
    """Detect 3x3 plus/cross patterns, expand arms to radius 2, place center at diagonals."""
    h = len(grid)
    w = len(grid[0]) if grid else 0
    out = [row[:] for row in grid]

    # Find all 3x3 cross centers
    crosses = []
    for r in range(1, h - 1):
        for c in range(1, w - 1):
            center = grid[r][c]
            if center == bg:
                continue
            # Check cardinal neighbors are same color (arm color)
            arm = grid[r - 1][c]
            if arm == bg or arm == center:
                continue
            if (grid[r + 1][c] == arm and grid[r][c - 1] == arm and
                    grid[r][c + 1] == arm):
                # Check diagonals are bg (true cross, not filled square)
                if (grid[r - 1][c - 1] == bg and grid[r - 1][c + 1] == bg and
                        grid[r + 1][c - 1] == bg and grid[r + 1][c + 1] == bg):
                    crosses.append((r, c, center, arm))

    for r, c, center_color, arm_color in crosses:
        # Extend arms to radius 2 (cardinal)
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nr, nc = r + 2 * dr, c + 2 * dc
            if 0 <= nr < h and 0 <= nc < w:
                out[nr][nc] = arm_color

        # Place center color at diagonal positions radius 1 and 2
        for dr, dc in [(-1, -1), (-1, 1), (1, -1), (1, 1)]:
            ...
            if 0 <= nr2 < h and 0 <= nc2 < w:
                out[nr2][nc2] = center_color

    return out


# ============================================================
# slide_toward_anchor — slide shape toward fixed anchor until adjacent
# ============================================================

def slide_toward_anchor(grid, mover_color=2, anchor_color=8, bg=0):
    """Slide all cells of mover_color toward anchor_color block until 4-adjacent."""
    h = len(grid)
    w = len(grid[0]) if grid else 0

    # Find mover and anchor positions
    mover_pos = set()
    anchor_pos = set()
    for r in range(h):
        for c in range(w):
            if grid[r][c] == mover_color:
                mover_pos.add((r, c))
            elif grid[r][c] == anchor_color:
                anchor_pos.add((r, c))

    if not mover_pos or not anchor_pos:
        return [row[:] for row in grid]

    # Compute centers
    mr = sum(r for r, c in mover_pos) / len(mover_pos)
    mc = sum(c for r, c in mover_pos) / len(mover_pos)
    ar = sum(r for r, c in anchor_pos) / len(anchor_pos)
    ac = sum(c for r, c in anchor_pos) / len(anchor_pos)
    ...
    # Slide until adjacent
    for _ in range(max(h, w)):
        # Check if any mover cell is adjacent to any anchor cell
        adjacent = False
        for pr, pc in mover_pos:
            for ddr, ddc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                if (pr + ddr, pc + ddc) in anchor_pos:
                    adjacent = True
                    break
            if adjacent:
                break
        ...
        new_pos = set()
        for pr, pc in mover_pos:
            nr, nc = pr + step[0], pc + step[1]
            if not (0 <= nr < h and 0 <= nc < w):
                return [row[:] for row in grid]
            if (nr, nc) in anchor_pos:
                return [row[:] for row in grid]
            new_pos.add((nr, nc))
        mover_pos = new_pos
    ...
    for r in range(h):
        for c in range(w):
            if grid[r][c] == mover_color:
                out[r][c] = bg
    # Place mover at new positions
    for r, c in mover_pos:
        out[r][c] = mover_color
    return out
