...
### 7.2 `test21` live components (verified iter 25)
...
| `agent/conditions/` (5 matchers; P5 = 5) | `all_outputs_comm`, `inputs_vary`, `intra_pair_grids_differ`, `pair_grid_count_majority`, `test_output_missing` — the Slice-1 recognition vocabulary documented in §4. `CONDITION_REGISTRY` + `register` decorator in `__init__.py`; matchers auto-loaded on import. |
...
| `program/anti_unification.py` | exposes `anti_unify_pair_programs()` / `anti_unify_terms()` (CLAUDE.md §8 names the entry point generically as `unify()`; the live function names differ). Present but **unwired** on `test21` — `save_rule_to_ltm` has no call site (Slice-1 does not need synthesis — SLICE_1_LOOP.md §9; modules E–J / AU are OUT until Slice 2). NB: `program/__init__.py` currently re-exports a non-existent `anti_unify`, so `import program` raises — a latent bug, harmless only because no live module imports the package. |
| `agent/active_operators.py` | pipeline operators; the `_try_*` / `_apply_*` family is retired (P6). `DescendOperator` (module A) and goal-evolution (module B) remain stubs — the impasse-driven descent flow is not yet wired (the deepest remaining capability gap; not required by SLICE_1_LOOP.md §8's relaxed criteria). |
| `tests/` (13 modules) | `test_compare_scheduler`, `test_conditions_*` (5 — one per matcher), `test_dsl`, `test_episodic_writer`, `test_fast_path_reuse`, `test_predict_copy_common_output`, `test_reconstruct_via_dsl`, `test_validate_rule`, `test_validate_rule_reuse` |
