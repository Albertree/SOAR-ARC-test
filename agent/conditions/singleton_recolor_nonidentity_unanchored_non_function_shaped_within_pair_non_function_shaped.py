...
pair_non_function_shaped -- match tasks where every group of every
example pair has BOTH ``len(set(input_colors)) == 1`` AND
...
all groups across all pairs -- iter 225 territory) AND AT LEAST ONE
pair has WITHIN-pair NON-function-shape (some C_g sees >= 2 distinct
K_g values within that pair's OWN groups).

The strict-disjoint complement of iter 226 (``singleton_recolor_
nonidentity_unanchored_non_function_shaped_within_pair_function_
shaped``) within iter 225's territory. The "non-function-shape lives
INSIDE at least one pair" sub-cell named by iter 226's "Next gap"
candidate (xvii).

Recognition vocabulary axis: the within-pair non-function-shape
sub-cell of iter 225's non-function-shape territory. Iter 225 names
the (F, F) "neither anchored" non-function-shape territory globally;
within that territory the per-pair (C_g, K_g) cross-product can be
either:
  ...
  * FUNCTION-shaped within each pair (every C_g sees exactly one K_g
    ...
    K_g_2 != K_g_1 in pair 2). Iter 226's matcher.
  * NON-FUNCTION-shaped within at least one pair (some C_g sees
    multiple K_g's within a single pair's own groups). THIS matcher.

The two cells together partition iter 225's territory exhaustively
along the within-pair function-shape sub-axis. Iter 226 names the
within-pair function-shape cell; this matcher names the within-pair
non-function-shape cell.

This matcher names the precondition for the rule family "whole-task
strict-recolour where at least one pair has within-pair non-function-
shape, blocking even the per-pair finite mapping table abstraction"
-- rules that require per-group context within each pair to
disambiguate K_g, beyond the bare (C_g) input. The companion sub-
cell to iter 226 within iter 225's territory: where iter 226's
sub-cell admits per-pair finite mapping tables that vary across
pairs, this sub-cell blocks even that per-pair table collapse
because the within-pair (C_g, K_g) cross-product is itself not a
function within at least one pair.
...
225's within-pair function-shape sub-axis):
  ...
  * Iter 226 (``singleton_recolor_nonidentity_unanchored_non_function_
    ...
    Iter 226 demands per-pair within-pair function-shape on every
    pair; this matcher demands within-pair non-function-shape on at
    least one pair. By definition disjoint.
...
Strict relations to iter 225 (the immediate parent):
  ...
  * Iter 225 (``singleton_recolor_nonidentity_unanchored_non_function_
    ...
    within-pair non-function-shape on any pair implies global non-
    function-shape on the union, since the union contains that pair
    as a subset). The converse fails on any iter-225 task where every
    pair is within-pair function-shape (iter 226 territory).
...
function-shape sub-axis):
  ...
  * Iter 224 (``singleton_recolor_nonidentity_unanchored_function_
    ...
    mutual exclusion with iter 224 on iter 223's territory; iter 224
    demands GLOBAL function-shape, this matcher demands GLOBAL non-
    function-shape).
...
iter 199). This matcher's territory has within-pair non-function-
shape: some C_g sees multiple K_g's within a single pair. If a
constant per-pair shift held, every C_g would map to exactly one
K_g within the pair (a function determined by the shift), so iter
198 would force within-pair function-shape, contradicting this
matcher's territory. Hence iter 198 / 199 typically REJECT on this
matcher's territory.
...
``within_pair_function_shaped: False`` flag: the matcher contract
...
not a name+params tuple. The within-pair non-function-shape sub-cell
of iter 225 is structurally identifiable in its own right -- it
names the rule family that blocks even the per-pair finite mapping
table abstraction (in contrast to iter 226's sub-cell, where each
pair admits its own table). Keeping the conjunction as a separate
registry slot lets anti-unification (CLAUDE.md §8) attach the right
gate per rule family -- specifically the gate for rules whose action
expression must consult per-group context within each pair to
disambiguate K_g -- without rediscovering the iter 225 ^ within-
pair-non-function-shape intersection from the rule's stored action
shape.

Why this matters as the within-pair non-function-shape sub-cell of
iter 225's territory: the within-pair non-function-shape claim is
the operational barrier that blocks anti-unification from collapsing
the per-pair (C_g, K_g) substitution into a per-pair finite mapping
table within at least one pair. The companion iter 226 sub-cell
admits per-pair table collapse (one table per pair, abstracted into
a family of tables). This sub-cell admits NEITHER global nor per-
pair table collapse -- requiring per-group context within each pair
where the within-pair non-function-shape lives. Naming this sub-cell
at a disjoint recognition handle lets anti-unification know in
advance that the per-pair mapping-table action shape is NOT
admissible, without having to scan each pair's per-group records
to discover the within-pair non-function-shape ad-hoc.
...
function-shape AND per-pair WITHIN-pair non-function-shape on at
least one pair):

  * Iter 225 (``singleton_recolor_nonidentity_unanchored_non_function_
    shaped``) -- STRICT REFINEMENT (this matcher additionally
    requires within-pair non-function-shape on at least one pair).
  * Iter 226 (``singleton_recolor_nonidentity_unanchored_non_function_
    shaped_within_pair_function_shaped``) -- STRICT MUTUAL EXCLUSION
...
Why this matters for ARBOR's intended ruleset:
  ...
  * "Within-pair non-function-shape strict-recolour" rule family --
    rules whose action must consult per-group context within each
    pair (position, shape, group index, neighbouring colours, ...)
    to disambiguate the K_g target, because the bare (C_g) input
    does not uniquely determine K_g even within a single pair. The
    hardest sub-cell of iter 225's territory: blocks both the global
    mapping table abstraction (iter 224 / iter 8 sub-cell) AND the
    per-pair mapping table abstraction (iter 226 sub-cell). The
    natural escalation cell where anti-unification must reach for
    per-group features beyond C_g (the per-group features available
    in the patterns dict: top_row, top_col, cell_count, group index
    within pair, ...).
  * Closes the iter-226-named candidate (xvii): "the strict-disjoint
    complement of iter 226 within iter 225's territory; names the
    precondition for the rule family 'whole-task strict-recolour
    where at least one pair has within-pair non-function-shape,
    blocking even the per-pair mapping table abstraction'". With
    this matcher landed, both cells of iter 225's within-pair
    function-shape sub-axis are named at disjoint recognition
    handles (iter 226 / this matcher), exhaustively partitioning
    iter 225's territory on that sub-axis.

Params:
  (none) -- pure per-pair within-pair non-function-shape existence
  check on iter 225's territory, universal over groups and pairs.
...
Returns True iff:
  ...
  - every entry of ``input_colors`` and ``output_colors`` is a
    ...
    / 200-206 / 213-226 strict-type posture), AND
  ...
  - the GLOBAL (C_g, K_g) cross-product across the union of all
    ...
    AND
  - AT LEAST ONE pair has WITHIN-pair NON-function-shape (some C_g
    in that pair sees >= 2 distinct K_g values within the pair's
    own groups -- the strict-disjoint complement of iter 226's
    universal within-pair function-shape claim).

Why fail-closed on empty / no-group / malformed (same posture as
iter 8 / 13 / 14 / 18 / 30-39 / 184-226): a missing or zero-group
pair is upstream extractor breakage or iter 13's no-blob-identity
territory; a within-pair-non-function-shaped strict-recolour claim
with zero observations is meaningless, AND admitting it would
collapse this matcher's territory into iter 13's.
...
def match(patterns: dict, params: dict) -> bool:
    ...
    # some C_g in that pair sees >= 2 distinct K_g values within the
    # pair's own groups (this matcher's distinguishing claim vs iter
    # 226).
    ...
    for analysis in pair_analyses:
        ...
        for group in groups:
            ...
            if mapped is None:
                ...
                # Within-pair non-function-shape on this pair: this
                # matcher's distinguishing claim vs iter 226. Mark and
                # continue scanning so the iter 225 preconditions
                # (observed cardinality, global non-function-shape,
                # per-group / strict-type structural gates) are still
                # checked over every remaining group / pair.
    ...
    # 225 / iter 226 contract and to keep this matcher self-contained.
