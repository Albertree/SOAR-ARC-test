...
def try_chunked_rules(comparisons: dict, task, patterns: dict = None) -> dict:
    ...
    # Sort: merged rules first (higher validation counts), then by times_validated
    all_rules.sort(key=lambda r: r.get("times_validated", 0), reverse=True)
...
def _find_duplicate(topology: dict, concept_id: str):
    ...
    for f in sorted(os.listdir(CHUNK_DIR)):
        ...
        if topologies_match(topology, existing_topo) and existing_concept == concept_id:
