...
def try_claude_solve(task, patterns, comparisons, procedural_root="procedural_memory"):
    ...
    # Snapshot existing concepts before calling Claude (track mtimes to detect edits)
    before = {p: os.path.getmtime(p) for p in glob.glob(os.path.join(concepts_dir, "*.json"))}
    ...
    # Check for new or modified concepts Claude wrote directly to disk
    after_paths = set(glob.glob(os.path.join(concepts_dir, "*.json")))
    after_mtimes = {p: os.path.getmtime(p) for p in after_paths}
    new_files = after_paths - set(before.keys())
    modified_files = {p for p in after_paths & set(before.keys())
                      if after_mtimes[p] > before[p]}
    ...
    if response:
        ...
        if concept and concept.get("concept_id"):
            ...
            if concept_path not in changed_files:
                with open(concept_path, "w") as f:
                    json.dump(concept, f, indent=2)
                changed_files.add(concept_path)
    ...
    if modified_files:
        print(f"[CLAUDE] {len(modified_files)} concept(s) modified")
    print(f"[CLAUDE] {len(changed_files)} new/modified concept(s) found")
...
def _reload_and_match(patterns, task):
    ...
    try:
        from procedural_memory.base_rules._concept_engine import _clear_cache
        _clear_cache()
    except Exception:
        pass
