...
class GeneralizeOperator(Operator):
    ...
    def effect(self, wm):
        ...
        # Strategy: corner block inward (corner blocks of majority color shift inward)
        if rule is None:
            rule = self._try_corner_block_inward(patterns, task)

        # Strategy: largest blob extract (output = small uniform grid of largest component color)
        if rule is None:
            rule = self._try_largest_blob_extract(patterns, task)
    ...
    # ---- strategy: corner block inward ------------------------------------

    def _try_corner_block_inward(self, patterns, task):
        """
        Detect: grid is mostly one bg color with rectangular blocks at corners.
        Blocks whose color appears at multiple corners shift inward by their
        block dimensions.  Blocks with a unique corner-color stay in place.
        Category: corner-marker reflection / inward-shift tasks.
        """
        if task is None:
            return None
        if not patterns.get("grid_size_preserved"):
        ...
        for pair in task.example_pairs:
            ig, og = pair.input_grid.raw, pair.output_grid.raw
            h, w = len(ig), len(ig[0])
            if len(og) != h or len(og[0]) != w:
                return None

            bg = Counter(c for row in ig for c in row).most_common(1)[0][0]

            corners = self._detect_corner_blocks(ig, bg, h, w)
            if corners is None or len(corners) < 2:
                return None

            predicted = self._corner_block_inward_grid(ig, bg, h, w, corners)
            if predicted != og:
                return None

        return {"type": "corner_block_inward", "confidence": 0.95}

    @staticmethod
    def _detect_corner_blocks(grid, bg, h, w):
        """Return list of (corner_label, color, r0, c0, r1, c1) for each
        corner that has a rectangular non-bg block."""
        results = []
        for label, start_r, start_c, dr, dc in [
            ("TL", 0, 0, 1, 1),
            ("TR", 0, w - 1, 1, -1),
            ("BL", h - 1, 0, -1, 1),
            ("BR", h - 1, w - 1, -1, -1),
            ...
            bh = 0
            r = start_r
            while 0 <= r < h and grid[r][start_c] == color:
                bh += 1
                r += dr
            # Measure block width
            bw = 0
            c = start_c
            while 0 <= c < w and grid[start_r][c] == color:
                bw += 1
                c += dc
            # Verify full rectangle
            r0 = min(start_r, start_r + dr * (bh - 1))
            r1 = max(start_r, start_r + dr * (bh - 1))
            c0 = min(start_c, start_c + dc * (bw - 1))
            c1 = max(start_c, start_c + dc * (bw - 1))
            ok = True
            for rr in range(r0, r1 + 1):
                ...
                if not ok:
                    break
            if ok:
                results.append((label, color, r0, c0, r1, c1))
        return results if results else None

    @staticmethod
    def _corner_block_inward_grid(grid, bg, h, w, corners):
        """Produce the output grid by shifting majority-color corner blocks inward."""
        # Count how many corners each color occupies
        color_count = Counter(color for _, color, *_ in corners)
        # Colors at >1 corner move; colors at 1 corner stay
        move_colors = {c for c, n in color_count.items() if n > 1}

        out = [[bg] * w for _ in range(h)]

        for label, color, r0, c0, r1, c1 in corners:
            bh = r1 - r0 + 1
            bw = c1 - c0 + 1
            if color in move_colors:
                # Compute inward shift
                if label == "TL":
                    nr0, nc0 = r0 + bh, c0 + bw
                elif label == "TR":
                    nr0, nc0 = r0 + bh, c1 - bw - bw + 1
                elif label == "BL":
                    nr0, nc0 = r1 - bh - bh + 1, c0 + bw
                else:  # BR
                    nr0, nc0 = r1 - bh - bh + 1, c1 - bw - bw + 1
                for rr in range(bh):
                    for cc in range(bw):
                        pr, pc = nr0 + rr, nc0 + cc
    ...
    # ---- strategy: largest blob extract -----------------------------------

    def _try_largest_blob_extract(self, patterns, task):
        """
        Detect: noisy multi-color grid with a few large solid-color blobs.
        Output is a small uniform grid filled with the color of the largest
        blob (connected component).
        Category: denoise / extract dominant region tasks.
        """
        if task is None:
            return None
        # Output must be smaller than input
        ...
        for pair in task.example_pairs:
            ...
            out_color = og[0][0]
            if not all(og[r][c] == out_color for r in range(oh) for c in range(ow)):
                return None
            # Output size must be consistent
            if out_h is None:
                out_h, out_w = oh, ow
            elif (oh, ow) != (out_h, out_w):
                return None
            # Largest connected component must match output color
            largest_color = self._find_largest_component_color(ig)
            if largest_color != out_color:
                return None

        return {"type": "largest_blob_extract", "out_h": out_h, "out_w": out_w,
                "confidence": 0.9}

    @staticmethod
    def _find_largest_component_color(grid):
        """Find the color of the largest dense 4-connected component.
        Dense means component_size / bounding_box_area >= 0.7, which
        filters out scattered background noise."""
        h = len(grid)
        w = len(grid[0]) if grid else 0
        visited = [[False] * w for _ in range(h)]
        best_size = 0
        ...
        for r in range(h):
            for c in range(w):
                ...
                if size < 10:
                    continue
                rows = [cr for cr, cc in cells]
                cols = [cc for cr, cc in cells]
                bbox_area = (max(rows) - min(rows) + 1) * (max(cols) - min(cols) + 1)
                density = size / bbox_area
                if density >= 0.7 and size > best_size:
                    best_size = size
                    best_color = color
        return best_color
...
class PredictOperator(Operator):
    ...
    def _apply_rule(self, rule, input_grid):
        ...
        if rule_type == "corner_block_inward":
            return self._apply_corner_block_inward(rule, input_grid)
        if rule_type == "largest_blob_extract":
            return self._apply_largest_blob_extract(rule, input_grid)
    ...
    def _apply_corner_block_inward(self, rule, input_grid):
        """Shift majority-color corner blocks inward by their dimensions."""
        raw = input_grid.raw
        h = len(raw)
        w = len(raw[0]) if raw else 0
        bg = Counter(c for row in raw for c in row).most_common(1)[0][0]
        corners = GeneralizeOperator._detect_corner_blocks(raw, bg, h, w)
        if not corners:
            return [row[:] for row in raw]
        return GeneralizeOperator._corner_block_inward_grid(raw, bg, h, w, corners)

    def _apply_largest_blob_extract(self, rule, input_grid):
        """Return a small uniform grid filled with the largest component color."""
        raw = input_grid.raw
        color = GeneralizeOperator._find_largest_component_color(raw)
        out_h = rule["out_h"]
        out_w = rule["out_w"]
