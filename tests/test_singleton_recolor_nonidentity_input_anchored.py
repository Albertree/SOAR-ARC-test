"""
tests/test_singleton_recolor_nonidentity_input_anchored.py -- exercise
the iter-221 matcher
``agent.conditions.singleton_recolor_nonidentity_input_anchored``.

Pins the matcher's contract per the docstring of
``agent/conditions/singleton_recolor_nonidentity_input_anchored.py``:
...
Runs without pytest:
    ...
    python tests/test_singleton_recolor_nonidentity_input_anchored.py
...
MATCHER_NAME = "singleton_recolor_nonidentity_input_anchored"
...
def test_returns_true_on_input_anchored_two_distinct_K() -> None:
    # The canonical cell: single pair, two groups, same C=3, distinct K's.
...
def test_returns_true_with_three_groups_three_distinct_K() -> None:
    patterns = {"pair_analyses": [
        _pair([_group([3], [0]), _group([3], [7]), _group([3], [5])]),
    ]}
    assert _matcher()(patterns, {}) is True


def test_returns_true_with_K_varying_across_pairs() -> None:
    # Single group per pair; the K_g varies across pairs (input anchor
    # is C=3 globally; outputs differ pair-to-pair).
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0])]),
            _pair([_group([3], [7])]),
...
def test_returns_true_with_K_varying_within_and_across_pairs() -> None:
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
            _pair([_group([3], [1])]),
...
def test_returns_true_with_duplicate_entries_in_color_lists() -> None:
    ...
    patterns = {"pair_analyses": [
        _pair([_group([4, 4], [7, 7]), _group([4], [2])]),
    ]}
    assert _matcher()(patterns, {}) is True


def test_returns_true_on_input_zero() -> None:
    # Edge: input C == 0, outputs K_g varying.
    patterns = {"pair_analyses": [
...
def test_returns_true_on_output_nine() -> None:
    # Edge: one K_g == 9 (highest valid colour); other K_g != 9.
    patterns = {"pair_analyses": [
        _pair([_group([0], [9]), _group([0], [3])]),
    ]}
    assert _matcher()(patterns, {}) is True
...
def test_returns_false_on_iter_220_territory_K_globally_uniform() -> None:
    # KEY iter-220 mutual-exclusion witness: iter 220 demands output-
    # side cross-group identity. This matcher demands its failure
    # (|observed_output| > 1). Iter 220 fires here; this matcher
    # rejects.
    patterns = {"pair_analyses": [
...
def test_returns_false_when_input_singletons_differ_across_groups() -> None:
    ...
    # input-side cross-group identity, rejecting any task where C_g
...
def test_returns_false_when_input_singletons_differ_across_pairs() -> None:
    # Input-side cross-group identity must hold cross-pair too.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0])]),
            _pair([_group([5], [7])]),
...
def test_returns_false_when_single_group_only() -> None:
    # A single group means |observed_output_colors| == 1, hence iter
    # 220's territory (not this matcher's). This matcher needs at
    # least two distinct K_g witnesses.
...
def test_returns_false_when_one_of_many_groups_is_identity() -> None:
    ...
    # oc == [C]) violates the per-group ic != oc demand on that group.
    patterns = {"pair_analyses": [
        _pair([
            _group([3], [0]),
...
def test_returns_false_when_any_pair_breaks_input_anchor() -> None:
    # A single failing pair breaks the whole task.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
            _pair([_group([5], [0]), _group([5], [7])]),  # different C
        ],
    }
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_groups_empty_on_any_pair() -> None:
    ...
    # singleton-blob input-anchored / output-per-group strict-recolour
...
def test_returns_false_when_color_out_of_range() -> None:
    ...
    # 215 / 217 / 218 / 219 / 220's strict-range gate).
...
def test_strict_refinement_of_iter_218_nonidentity_per_group() -> None:
    ...
    # Converse fails when iter 218 fires WITHOUT input-side cross-
    # group identity (per-group C_g varying across groups).
    iter218 = CONDITION_REGISTRY["singleton_recolor_nonidentity_per_group"]

    # This matcher fires => iter 218 fires.
    p1 = {"pair_analyses": [_pair([_group([3], [0]), _group([3], [7])])]}
...
def test_strict_mutual_exclusion_with_iter_220_both_anchored() -> None:
    ...
    # named cells partitioning iter 218's input-anchored row at the
    # (output-anchored) vs (output-per-group) split.
    iter220 = CONDITION_REGISTRY["singleton_recolor_nonidentity"]

    # This matcher fires; iter 220 rejects (|observed_output| > 1 on
    # this matcher; iter 220 demands |observed_output| == 1).
    p1 = {"pair_analyses": [_pair([_group([3], [0]), _group([3], [7])])]}
    assert _matcher()(p1, {}) is True and iter220(p1, {}) is False

    # Iter 220 fires; this matcher rejects (|observed_output| == 1
...
def test_strict_mutual_exclusion_with_iter_216_both_anchored() -> None:
    ...
    # matcher demands output-side cross-group identity failure.
    # Disjoint by the output-anchor presence/absence split.
...
def test_strict_refinement_of_iter_215_singleton_recolor_per_group() -> None:
    ...
    # additionally demands per-group ic != oc AND input-side cross-
...
def test_strict_refinement_of_iter_14_input_color_uniform() -> None:
    # Iter 14 demands all input singletons across all groups bit-
    # identical to one global C. This matcher additionally requires
    # per-group |oc| == 1 AND per-group ic != oc AND
    # |observed_output| > 1.
    ...
    # Iter 14 fires with multi-output per group (iter 14 doesn't
    # constrain output side); this matcher rejects.
...
def test_strict_mutual_exclusion_with_iter_18_output_color_uniform() -> None:
    ...
    # this matcher demands |observed_output| > 1. Disjoint.
...
def test_strict_mutual_exclusion_with_iter_8_consistent_color_mapping() -> None:
    # Iter 8 demands a function-shaped global mapping. When this
    # matcher fires, the per-pair (ic, oc) cross-product contains
    # multiple (C, K_g) pairs with the same C and distinct K_g,
    # violating function-shape on input C. So iter 8 rejects this
    # matcher's distinctive territory.
...
def test_strict_implication_of_iter_203_disjoint_per_group() -> None:
    # Iter 203 demands per-group set(ic) ∩ set(oc) == ∅. Strict
    # implication: this matcher fires => iter 203 fires (per-group
    # ic = [C] / oc = [K_g] with K_g != C; on the singleton row this
    # forces ic ∩ oc == ∅).
...
def test_strict_mutual_exclusion_with_iter_201_set_equality_per_group() -> None:
    ...
    # pins per-group ic == [C] / oc == [K_g] with K_g != C, forcing
...
def test_strict_refinement_of_iter_213_consistent_color_mapping_per_group() -> None:
    ...
    # additionally requires per-group |ic| == 1 AND input-side cross-
    # group identity AND per-group ic != oc AND
    # |observed_output| > 1.
...
def test_strict_refinement_of_iter_214_input_color_uniform_per_group() -> None:
    ...
    # additionally requires per-group |oc| == 1 AND input-side cross-
    # group identity AND per-group ic != oc AND
    # |observed_output| > 1.
...
def test_strict_refinement_of_iter_197_at_K_prod_eq_1() -> None:
    ...
    # additionally requires the input-anchored / output-per-group
    ...
    # (output anchored); this matcher rejects.
    p2 = {
        "pair_analyses": [
            _pair([_group([3], [0])]),
            _pair([_group([3], [0])]),
...
def test_recognized_conditions_includes_matcher() -> None:
    ...
    assert MATCHER_NAME in fired, (
        f"{MATCHER_NAME!r} did not fire on an input-anchored / output-"
        f"per-group strict-recolour patterns dict; got {fired!r}"
    )
    ...
    assert "singleton_recolor_nonidentity" not in fired, (
        "iter 220 should not fire on input-anchored / output-per-group "
        f"territory; got {fired!r}"
    )
...
def test_recognized_conditions_excludes_on_iter_220_territory() -> None:
    ...
    # (output-side cross-group identity, |observed_output| == 1).
    from agent.conditions import recognized_conditions
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [0])]),
