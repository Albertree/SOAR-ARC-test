...
shaped_within_pair_non_function_shaped_mixed.py -- exercise the
iter-229 matcher ``agent.conditions.singleton_recolor_nonidentity_
...
function_shaped_within_pair_non_function_shaped_mixed.py``:
...
values across the union -- iter 225 territory) AND AT LEAST ONE pair
has WITHIN-pair NON-function-shape (iter 227 existential claim) AND
AT LEAST ONE pair has WITHIN-pair FUNCTION-shape (the strict-
complement of iter 228's universal-pair claim).

Universal over groups; mixed-shape across pairs (existential on
both within-pair function-shape and non-function-shape); fail-
closed on empty / no-group / malformed input.

Runs without pytest:

    python tests/test_singleton_recolor_nonidentity_unanchored_non_function_shaped_within_pair_non_function_shaped_mixed.py
...
def test_returns_true_on_canonical_mixed_shape_two_pairs() -> None:
    # KEY canonical mixed-shape fixture: pair 1 within-pair non-function-
    # shape (C=3 -> {0, 7}); pair 2 within-pair function-shape (3 -> 0,
    # 5 -> 4). |observed_input| = {3, 5} > 1; |observed_output| =
    # {0, 4, 7} > 1; global non-function-shape (C=3 sees {0, 7} across
    # the union).
    patterns = {
        "pair_analyses": [
...
def test_returns_true_when_function_shape_pair_is_last() -> None:
    # Pair 1 / 2 within-pair non-function-shape; pair 3 within-pair
    # function-shape. Iter 227 fires (existential); iter 228 rejects
    # (universal demand fails on pair 3); this matcher fires (mixed-
    # shape).
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
...
def test_returns_true_when_function_shape_pair_is_first() -> None:
    # Pair 1 within-pair function-shape; pair 2 / 3 within-pair non-
    # function-shape. Mixed-shape.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
            _pair([_group([3], [0]), _group([3], [7])]),
...
def test_returns_true_with_three_pairs_two_function_one_non_function() -> None:
    # Two within-pair function-shape pairs + one within-pair non-
    # function-shape pair. Both existentials hold.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
...
def test_returns_true_with_duplicate_entries_in_color_lists() -> None:
    # Colour lists are de-duplicated set-wise; the mixed-shape claim is
    # on the unique (C, K) pairs.
    patterns = {
        "pair_analyses": [
            _pair([_group([3, 3], [0, 0]), _group([3], [7])]),
            _pair([_group([3, 3], [0, 0]), _group([5, 5], [4, 4])]),
        ],
    }
    assert _matcher()(patterns, {}) is True
...
def test_returns_true_on_color_zero_and_nine_boundary_witnesses() -> None:
    # Edge: C=0 (low boundary) collides within pair 1 (non-function-
    # shape); C=9 (high boundary) is the function-shape pair's anchor.
    patterns = {
        "pair_analyses": [
            _pair([_group([0], [3]), _group([0], [9])]),
...
def test_returns_true_with_extra_groups_around_within_pair_shapes() -> None:
    # Each pair has additional non-distinguishing groups; the mixed-
    # shape claim still holds on the union.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7]), _group([2], [6])]),
            _pair([_group([3], [0]), _group([5], [4]), _group([2], [6])]),
        ],
    }
    assert _matcher()(patterns, {}) is True


def test_returns_true_when_function_shape_pair_has_only_one_group() -> None:
    # The within-pair function-shape pair has a single group (trivially
    # within-pair function-shape: no within-pair collision possible).
    # The within-pair non-function-shape pair carries the iter 227
    # existential.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
...
def test_returns_false_on_iter_228_universal_pair_canonical_fixture() -> None:
    # KEY iter-228 mutual-exclusion witness: every pair is within-pair
    # non-function-shape. Iter 227 / iter 228 fire; this matcher
    # rejects (no within-pair function-shape pair exists).
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
            _pair([_group([5], [1]), _group([5], [4])]),
...
def test_returns_false_on_single_pair_within_pair_non_function_shape() -> None:
    # KEY single-pair witness: only one pair, within-pair non-function-
    # shape. Iter 227 / iter 228 fire (universal over a single pair
    # reduces to existential); this matcher rejects (no within-pair
    # function-shape pair exists with only one pair total).
    patterns = {
        "pair_analyses": [
...
def test_returns_false_on_iter_226_canonical_fixture() -> None:
    ...
    # function-shape. Iter 226 territory; iter 227 (and hence this
    # matcher) rejects (no within-pair non-function-shape pair).
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
            _pair([_group([3], [7]), _group([5], [4])]),
...
def test_returns_false_on_collision_only_across_pairs() -> None:
    ...
    # collision exists. Iter 226 territory; this matcher rejects.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0])]),
            _pair([_group([3], [7])]),
...
def test_returns_false_on_iter_10_canonical_function_shape_fixture() -> None:
    ...
    # 228 / this matcher all reject (global non-function-shape fails).
...
def test_returns_false_on_global_function_shape_with_multi_pair() -> None:
    ...
    # (iter 224 territory). This matcher rejects (global non-function-
    # shape fails).
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
...
def test_strict_mutual_exclusion_with_iter_228_universal_pair() -> None:
    # KEY iter-228 mutual-exclusion witness: iter 228 demands EVERY pair
    # within-pair non-function-shape; this matcher demands SOME pair
    # within-pair function-shape. Disjoint by definition.
    iter228 = CONDITION_REGISTRY[
        "singleton_recolor_nonidentity_unanchored_non_function_shaped_"
        "within_pair_non_function_shaped_universal"
    ]

    # Iter 228 fires; this matcher rejects.
    p1 = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
            _pair([_group([5], [1]), _group([5], [4])]),
    ...
    assert iter228(p1, {}) is True and _matcher()(p1, {}) is False

    # This matcher fires; iter 228 rejects (pair 2 within-pair function-
    # shape).
    p2 = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
    ...
    assert _matcher()(p2, {}) is True and iter228(p2, {}) is False
...
def test_strict_refinement_of_iter_227_within_pair_non_function_shape() -> None:
    ...
    # iter 227 fires. The converse fails on the universal-pair sub-
    # cell (iter 228 territory).
    ...
    # Iter 227 fires on the universal-pair sub-cell; this matcher
    # rejects (no within-pair function-shape pair).
    p2 = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
            _pair([_group([5], [1]), _group([5], [4])]),
...
def test_strict_mutual_exclusion_with_iter_226_within_pair_function_shaped() -> None:
    ...
    # pair within-pair function-shape; this matcher demands at least
    # one pair within-pair non-function-shape. Disjoint via iter 227.
    iter226 = CONDITION_REGISTRY[
        "singleton_recolor_nonidentity_unanchored_non_function_shaped_"
        "within_pair_function_shaped"
    ]

    # Iter 226 fires; this matcher rejects (no within-pair non-function-
    # shape pair).
    p1 = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
    ...
    # This matcher fires; iter 226 rejects (pair 1 within-pair non-
    # function-shape).
    p2 = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
...
def test_recognized_conditions_includes_matcher_on_mixed_shape() -> None:
    # On the mixed-shape sub-cell of iter 227, this matcher FIRES.
    # Iter 227 must co-fire (strict refinement). Iter 228 must REJECT
    # (universal-pair demand fails). Iter 226 must REJECT (within-pair
    # function-shape on every pair fails). Iter 225 must co-fire.
    # Iter 224 must REJECT. Iter 8 must REJECT. Iter 223 must co-fire.
    ...
    assert MATCHER_NAME in fired, (
        f"{MATCHER_NAME!r} did not fire on the mixed-shape sub-cell of "
        f"iter 227; got {fired!r}"
    )
    assert (
        "singleton_recolor_nonidentity_unanchored_non_function_shaped_"
        "within_pair_non_function_shaped"
    ...
    assert (
        ...
        "iter 228 expectation: universal-pair matcher must NOT fire "
        f"on the mixed-shape sub-cell; got {fired!r}"
    )
    assert (
        "singleton_recolor_nonidentity_unanchored_non_function_shaped"
        in fired
...
def test_recognized_conditions_excludes_on_universal_pair_sub_cell() -> None:
    # The companion universal-pair sub-cell of iter 227 (every pair
    # within-pair non-function-shape) is iter 228 territory. Iter 225 /
    # iter 227 / iter 228 fire; iter 226 rejects; this matcher must
    # NOT fire (no within-pair function-shape pair).
    from agent.conditions import recognized_conditions
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
    ...
    assert MATCHER_NAME not in fired, (
        f"{MATCHER_NAME!r} should NOT fire on the universal-pair sub-"
        f"cell of iter 227; got {fired!r}"
    )
    # No regression on iter 227 / 228 / 225 / 223.
    assert (
        "singleton_recolor_nonidentity_unanchored_non_function_shaped_"
        "within_pair_non_function_shaped"
        in fired
    ...
    assert "singleton_recolor_nonidentity_unanchored" in fired
    # Iter 226 must reject the universal-pair fixture (every pair
    # within-pair non-function-shape disqualifies iter 226's universal
    # claim on within-pair function-shape).
    assert (
        "singleton_recolor_nonidentity_unanchored_non_function_shaped_"
        "within_pair_function_shaped"
        not in fired
