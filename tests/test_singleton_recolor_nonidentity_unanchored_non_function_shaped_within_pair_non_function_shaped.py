...
shaped_within_pair_non_function_shaped.py -- exercise the iter-227
matcher ``agent.conditions.singleton_recolor_nonidentity_unanchored_
non_function_shaped_within_pair_non_function_shaped``.

Pins the matcher's contract per the docstring of
``agent/conditions/singleton_recolor_nonidentity_unanchored_non_
function_shaped_within_pair_non_function_shaped.py``: every group of
...
values across the union -- iter 225 territory) AND AT LEAST ONE
pair has WITHIN-pair NON-function-shape (some C_g sees >= 2
distinct K_g values within that pair's own groups -- the strict-
disjoint complement of iter 226's universal within-pair function-
shape claim).

Universal over groups; existential over pairs on the within-pair
non-function-shape claim; fail-closed on empty / no-group /
malformed input.

Runs without pytest:

    python tests/test_singleton_recolor_nonidentity_unanchored_non_function_shaped_within_pair_non_function_shaped.py
...
def test_returns_true_on_canonical_within_pair_non_function_shape() -> None:
    # Canonical fixture: pair 1 contains C=3 -> 0 AND C=3 -> 7 within
    # its own groups (within-pair non-function-shape on pair 1).
    # |observed_input| = {3, 5} > 1; |observed_output| = {0, 4, 7} > 1.
    patterns = {
        "pair_analyses": [
...
def test_returns_true_when_collision_lives_inside_one_pair() -> None:
    # The iter-225 fixture where the collision lives WITHIN one pair
    # (line 232-243 of tests/test_singleton_recolor_nonidentity_
    # unanchored_non_function_shaped_within_pair_function_shaped.py
    # -- iter 226 rejected on this fixture). This matcher accepts.
    patterns = {
        "pair_analyses": [
...
def test_returns_true_on_iter_225_canonical_single_pair_collision() -> None:
    # Single-pair iter-225 canonical fixture: C=3 -> {0, 7} within
    # pair 1 (within-pair non-function-shape). Iter 225 fires; iter
    # 226 rejects; this matcher fires.
    patterns = {
        "pair_analyses": [
...
def test_returns_true_when_only_one_pair_has_within_pair_collision() -> None:
    # Pair 1 within-pair non-function-shape (C=3 -> {0, 7}); pair 2
    # within-pair function-shape (3 -> 0, 5 -> 4). At least one pair
    # qualifies; this matcher fires.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
...
def test_returns_true_when_every_pair_has_within_pair_collision() -> None:
    # Every pair has within-pair non-function-shape (universal
    # quantifier on the existential claim still holds).
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
...
def test_returns_true_with_three_distinct_C_one_colliding_within_pair() -> None:
    # Three distinct C's in pair 1: 3 -> 0, 5 -> 4, 3 -> 9 (C=3 sees
    # both 0 and 9 within pair 1). |observed_input| = {3, 5},
    # |observed_output| = {0, 4, 9}.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4]), _group([3], [9])]),
        ],
    }
    assert _matcher()(patterns, {}) is True
...
def test_returns_true_with_duplicate_entries_in_color_lists() -> None:
    # Colour lists are de-duplicated set-wise; the within-pair non-
    # function-shape claim is on the unique (C, K) pairs.
    patterns = {
        "pair_analyses": [
            _pair([_group([3, 3], [0, 0]), _group([3], [7]), _group([5], [4])]),
        ],
    }
    assert _matcher()(patterns, {}) is True
...
def test_returns_true_on_color_zero_and_nine_boundary_witnesses() -> None:
    # Edge: C=0 (low boundary) sees both K=3 and K=9 within pair 1.
    patterns = {
        "pair_analyses": [
...
def test_returns_false_on_iter_226_canonical_fixture() -> None:
    ...
    # function-shape (3 -> 0, 5 -> 4 in pair 1; 3 -> 7, 5 -> 4 in
    # pair 2). Global non-function-shape on C=3 across pairs (iter
    # 226 territory). This matcher REJECTS (no pair has within-pair
    # collision).
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
...
def test_returns_false_on_global_function_shape_with_multi_pair() -> None:
    # Both pairs identical, both within-pair function-shape; union
    # also function-shape (iter 224 territory). This matcher rejects.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
            _pair([_group([3], [0]), _group([5], [4])]),
...
def test_returns_false_on_single_pair_function_shape_alone() -> None:
    ...
    # Iter 224 territory; iter 225 / this matcher reject.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
        ],
...
def test_returns_false_on_iter_221_territory_input_anchored() -> None:
    # Iter 221: cross-group identity on the INPUT side; |observed_
    # input| == 1 fails this matcher's precondition. (Note: this
    # fixture HAS within-pair non-function-shape on C=3 -> {0, 7},
    # but the iter 225 precondition fails on observed-input.)
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([3], [7])]),
        ],
...
def test_returns_false_when_any_group_has_multi_input() -> None:
    patterns = {
        "pair_analyses": [
            _pair([_group([3, 4], [0]), _group([3], [7]), _group([5], [4])]),
        ],
    }
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_any_group_has_multi_output() -> None:
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0, 4]), _group([3], [7]), _group([5], [4])]),
        ],
    }
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_input_colors_missing() -> None:
    ...
    patterns = {"pair_analyses": [analysis, _pair([_group([5], [4])])]}
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_color_list_contains_bool() -> None:
    ...
    patterns2 = {"pair_analyses": [analysis2, _pair([_group([5], [4])])]}
...
def test_strict_refinement_of_iter_225_non_function_shaped() -> None:
    ...
    # iter 225 fires. The converse fails on the within-pair function-
    # shape sub-cell of iter 225 (iter 226 territory).
...
def test_strict_mutual_exclusion_with_iter_226_within_pair_function_shaped() -> None:
    ...
    # one pair within-pair non-function-shape. Disjoint by definition.
...
def test_recognized_conditions_includes_matcher_on_within_pair_non_function_shape() -> None:
    # On the within-pair non-function-shape sub-cell of iter 225, this
    # matcher FIRES. Iter 225 must co-fire (strict refinement). Iter
    # 226 must REJECT (within-pair function-shape fails on at least
    # one pair). Iter 223 must co-fire. Iter 224 must REJECT (global
    # function-shape fails). Iter 8 must REJECT (global function-
    # shape fails).
    ...
    assert MATCHER_NAME in fired, (
        f"{MATCHER_NAME!r} did not fire on the within-pair non-"
        f"function-shape sub-cell of iter 225; got {fired!r}"
    )
    assert (
        "singleton_recolor_nonidentity_unanchored_non_function_shaped"
        in fired
...
def test_recognized_conditions_excludes_on_within_pair_function_shape() -> None:
    # The iter-226 canonical fixture (within-pair function-shape on
    # every pair) is iter 226 territory. Iter 225 / iter 226 fire;
    # this matcher must NOT fire.
    from agent.conditions import recognized_conditions
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [0]), _group([5], [4])]),
    ...
    assert MATCHER_NAME not in fired, (
        f"{MATCHER_NAME!r} should NOT fire on the within-pair function-"
        f"shape sub-cell; got {fired!r}"
    )
    # No regression on iter 225 / 226 / 223.
    assert (
        "singleton_recolor_nonidentity_unanchored_non_function_shaped"
        in fired
    )
