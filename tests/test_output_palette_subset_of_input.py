"""
tests/test_output_palette_subset_of_input.py -- exercise the iter-184
matcher ``agent.conditions.output_palette_subset_of_input`` (new in
this iter).

Pins the matcher's contract per
``agent/conditions/output_palette_subset_of_input.py`` docstring: every
...
Runs without pytest:
    ...
    python tests/test_output_palette_subset_of_input.py
...
MATCHER_NAME = "output_palette_subset_of_input"
...
def test_returns_true_when_output_palette_equals_input() -> None:
    # Identity-style: output palette equal to input.
    patterns = {"pair_analyses": [_pair([0, 1, 2], [0, 1, 2])]}
    assert _matcher()(patterns, {}) is True


def test_returns_true_when_output_palette_strict_subset() -> None:
    # Strict subset: output drops a colour present in input.
    patterns = {"pair_analyses": [_pair([0, 1, 2], [0, 1])]}
    assert _matcher()(patterns, {}) is True


def test_returns_true_on_single_color_grids() -> None:
    # Degenerate: both grids monochrome and the same colour.
    patterns = {"pair_analyses": [_pair([7], [7])]}
    assert _matcher()(patterns, {}) is True


def test_returns_true_across_multiple_pairs_with_varying_palettes() -> None:
    # Per-pair containment is independent across pairs; varying
...
def test_returns_true_when_output_palette_empty() -> None:
    # The empty set is a subset of every set. An empty output palette
    # is degenerate (zero-area output) but the gate must not fail it.
...
def test_returns_false_when_output_introduces_one_new_color() -> None:
    # A single new colour on the output side fails the gate.
...
def test_returns_false_when_any_pair_fails_the_gate() -> None:
    ...
    patterns = {
        "pair_analyses": [
            _pair([0, 1, 2], [0, 1]),
            _pair([0, 1], [0, 1, 9]),  # offending pair
            _pair([3], [3]),
        ],
    }
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_input_palette_contains_bool() -> None:
    ...
    # (same posture as the iter-182 / 183 dimensional matchers).
    analysis = _pair([0, 1], [0])
...
def test_ignores_per_group_color_lists() -> None:
    ...
    # Per-group ``input_colors`` / ``output_colors`` on the change cells
...
def test_ignores_dimensional_fields() -> None:
    ...
    analysis = _pair([0, 1], [0], input_height=7, input_width=9,
...
def test_implication_does_not_reverse() -> None:
    # A pure recolour (every red becomes blue, no new colour
    # introduced) fires this matcher but NOT identity_transformation
    # (cells are changed). Confirms the asymmetry.
    identity = CONDITION_REGISTRY["identity_transformation"]
    analysis = _pair(
        [0, 1], [0, 1],
        groups=[{
...
def test_orthogonal_to_grid_size_preserved() -> None:
    # Whole-grid palette membership is orthogonal to the per-pair
    ...
    # subset + changed -- only subset fires
    p2 = {
...
def test_orthogonal_to_input_color_uniform() -> None:
    ...
    # whole-grid palette axis is INDEPENDENT.
    icu = CONDITION_REGISTRY["input_color_uniform"]

    # subset fires AND icu fires (one source colour, output stays in
    # the input palette).
    analysis = _pair(
        [0, 5], [0, 5],
        groups=[{
    ...
    # subset fires but icu does NOT (no change cells means iter 14 is
    # vacuously False per its docstring: requires len(groups) >= 1).
    p2 = {"pair_analyses": [_pair([0, 1], [0, 1])]}
    assert _matcher()(p2, {}) is True and icu(p2, {}) is False


def test_recognized_conditions_includes_output_palette_subset_of_input() -> None:
    from agent.conditions import recognized_conditions
    patterns = {"pair_analyses": [_pair([0, 1, 2], [0, 1])]}
    fired = recognized_conditions(patterns)
    assert MATCHER_NAME in fired, (
        f"{MATCHER_NAME!r} did not fire on a clearly-subset patterns "
        f"dict; got {fired!r}"
    )
