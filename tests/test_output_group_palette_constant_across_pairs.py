"""
tests/test_output_group_palette_constant_across_pairs.py -- exercise
the iter-995 matcher
``agent.conditions.output_group_palette_constant_across_pairs`` (new
in this iter).

Pins the matcher's contract per
``agent/conditions/output_group_palette_constant_across_pairs.py``
docstring: every change group of every example pair shares the same
``frozenset(group["output_colors"])`` on a non-empty ``pair_analyses``
...
axis: the output-side dual of iter 994's
``input_group_palette_constant_across_pairs`` AND the per-group
projection of iter 990's whole-grid
``output_palette_constant_across_pairs`` on the output-side cross-
pair-set-constancy x per-group-scope cell.

Runs without pytest:

    python tests/test_output_group_palette_constant_across_pairs.py
...
MATCHER_NAME = "output_group_palette_constant_across_pairs"
...
def _group(output_colors, input_colors=None, **overrides):
    """A group_analysis shaped like ``_analyze_pair``'s emit (iter-1
    schema). ``input_colors`` defaults to a distinct singleton so the
    matcher cannot accidentally read input state."""
    ...
    base = {
        ...
        "top_row": 0,
        "top_col": 0,
        "cell_count": max(len(output_colors), 1),
        "positions": [(0, 0)],
    }
...
def test_p5_at_least_89() -> None:
    # This matcher brings the registry to >= 89 (P5 monotone).
    assert len(CONDITION_REGISTRY) >= 89, (
        f"expected >= 89 matchers post-iter-995; got {len(CONDITION_REGISTRY)}"
    )
...
def test_same_cardinality_different_set_rejects() -> None:
    # Iter 196's territory (constant cardinality, varying set): this
    # matcher must reject.
    patterns = {
        "pair_analyses": [
            _pair([_group([0, 1])]),
...
def test_empty_groups_list_rejects_identity_territory() -> None:
    ...
    # 195/196/197/207/208/994 per-group posture).
...
def test_empty_output_colors_list_rejects() -> None:
    # An empty per-group output_colors list is an extractor contract
    # violation (a connected change group always has >= 1 cell with a
    # non-null output colour).
    g = _group([0, 1])
    g["output_colors"] = []
    patterns = {"pair_analyses": [_pair([g])]}
    assert _matcher()(patterns, {}) is False
...
def test_ignores_input_colors() -> None:
    # The matcher reads ONLY per-group ``output_colors``. Different
    # input_colors must not affect the verdict.
    patterns = {
        "pair_analyses": [
            _pair([_group([0, 1], input_colors=[2])]),
            _pair([_group([0, 1], input_colors=[3])]),
        ],
    }
    assert _matcher()(patterns, {}) is True
...
def test_ignores_whole_grid_palette_fields() -> None:
    # The matcher reads per-group output_colors only; arbitrary
    # whole-grid ``input_palette`` / ``output_palette`` must not
    # change the verdict (distinguishing this matcher from iter 990).
    patterns = {
        "pair_analyses": [
...
def test_strict_implication_of_per_group_output_color_count() -> None:
    # Iter 196: per-group |output_colors| constancy. STRICT IMPLICATION:
    # this matcher implies iter 196 (equal sets have equal cardinality).
    ...
    # Constant cardinality 2 with varying sets: iter 196 fires,
    ...
    assert iter196(p_card_only, {}) is True


def test_independent_from_whole_grid_output_palette_constant() -> None:
    # Iter 990: whole-grid output palette cross-pair constancy. NOT in
    # a refinement relation either way.
    iter990 = CONDITION_REGISTRY["output_palette_constant_across_pairs"]

    # This matcher fires, iter 990 rejects: every change blob ends in
    # ``{0, 1}`` across both pairs, but the surrounding background
    # makes the whole-grid output palette differ.
    p_group_only = {
        "pair_analyses": [
            _pair([_group([0, 1])], output_palette=[0, 1, 2]),
            _pair([_group([0, 1])], output_palette=[0, 1, 3]),
        ],
    }
    assert _matcher()(p_group_only, {}) is True
    assert iter990(p_group_only, {}) is False

    # Iter 990 fires, this matcher rejects: whole-grid palette equal
    # across pairs, but per-group ``output_colors`` differs.
    p_whole_only = {
        "pair_analyses": [
            _pair([_group([0, 1])], output_palette=[0, 1, 2]),
            _pair([_group([1, 2])], output_palette=[0, 1, 2]),
        ],
    }
    assert _matcher()(p_whole_only, {}) is False
    assert iter990(p_whole_only, {}) is True
...
def test_independent_from_change_output_colors_constant_across_pairs() -> None:
    # Iter 36: per-pair-aggregated set of single-colour-blob output
    ...
    # iter 36 rejects (requires ``len == 1``).
    p_multi = {
        "pair_analyses": [
            _pair([_group([0, 1])]),
            _pair([_group([0, 1])]),
    ...
    assert iter36(p_multi, {}) is False

    # Iter 36 fires on a multi-blob task where per-pair {blob outputs}
    ...
    assert _matcher()(p_iter36_only, {}) is False
    assert iter36(p_iter36_only, {}) is True


def test_independent_from_input_group_palette_sibling() -> None:
    # Iter 994: input-side per-group sibling. INDEPENDENT in both
    # directions on the (input, output) axis.
    iter994 = CONDITION_REGISTRY["input_group_palette_constant_across_pairs"]

    # This matcher fires, iter 994 rejects: per-group output sets
    # constant across pairs/groups, but per-group input sets vary.
    p_out_only = {
        "pair_analyses": [
            _pair([_group([0, 1], input_colors=[2, 3])]),
            _pair([_group([0, 1], input_colors=[4, 5])]),
        ],
    }
    assert _matcher()(p_out_only, {}) is True
    assert iter994(p_out_only, {}) is False

    # Iter 994 fires, this matcher rejects: per-group input sets
    # constant, but per-group output sets vary.
    p_in_only = {
        "pair_analyses": [
            _pair([_group([2, 3], input_colors=[0, 1])]),
            _pair([_group([4, 5], input_colors=[0, 1])]),
        ],
    }
    assert _matcher()(p_in_only, {}) is False
    assert iter994(p_in_only, {}) is True

    # Both fire on simultaneously-pinned input/output per-blob palettes.
    p_both = {
        "pair_analyses": [
            _pair([_group([0, 1], input_colors=[2, 3])]),
            _pair([_group([0, 1], input_colors=[2, 3])]),
...
def test_does_not_displace_adjacent_iter_matchers() -> None:
    ...
    assert not missing, f"adjacent matchers missing post-iter-995: {missing!r}"
