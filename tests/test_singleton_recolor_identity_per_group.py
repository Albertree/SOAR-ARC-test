"""
tests/test_singleton_recolor_identity_per_group.py -- exercise the
iter-217 matcher ``agent.conditions.singleton_recolor_identity_per_
group``.

Pins the matcher's contract per the docstring of
``agent/conditions/singleton_recolor_identity_per_group.py``: every
group of every example pair has BOTH ``len(set(input_colors)) == 1``
AND ``len(set(output_colors)) == 1`` AND ``set(input_colors) ==
set(output_colors)`` -- the per-group singleton fixed-point cell, the
CO-FIRE conjunction of iter 215 (``singleton_recolor_per_group``)
AND iter 201 (``output_colors_equals_input_colors_per_group``).
Universal over groups AND pairs; fail-closed on empty / no-group /
malformed input.

Runs without pytest:

    python tests/test_singleton_recolor_identity_per_group.py
...
MATCHER_NAME = "singleton_recolor_identity_per_group"
...
def test_returns_true_on_per_group_singleton_fixed_point() -> None:
...
def test_returns_true_when_fixed_points_differ_across_groups() -> None:
    # Per-group fixed points may vary across groups (drops cross-
...
def test_returns_true_when_fixed_points_differ_across_pairs() -> None:
    # Per-group fixed points may vary across pairs too.
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [3])]),
            _pair([_group([7], [7])]),
...
def test_returns_true_with_multipair_fixed_point_groups() -> None:
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [3]), _group([7], [7])]),
            _pair([_group([3], [3]), _group([7], [7])]),
        ],
    }
    assert _matcher()(patterns, {}) is True
...
def test_returns_true_with_duplicate_entries_in_color_lists() -> None:
    ...
    patterns = {"pair_analyses": [
        _pair([_group([3, 3], [3, 3])]),
    ]}
    assert _matcher()(patterns, {}) is True


def test_returns_true_on_colour_zero_fixed_point() -> None:
    # Edge: fixed point at colour 0 (lowest valid colour) -- equally
    # admissible as any other singleton.
    patterns = {"pair_analyses": [
        _pair([_group([0], [0])]),
    ]}
    assert _matcher()(patterns, {}) is True
...
def test_returns_true_on_colour_nine_fixed_point() -> None:
    # Edge: fixed point at colour 9 (highest valid colour).
    patterns = {"pair_analyses": [
        _pair([_group([9], [9])]),
    ]}
    assert _matcher()(patterns, {}) is True
...
def test_returns_false_when_any_group_singleton_recolour_non_identity() -> None:
    # KEY strict-refinement witness vs iter 215: ic = [3], oc = [4].
    # Iter 215 fires (both sides singletons); this matcher rejects
    # (the singletons differ -- not a fixed point).
    patterns = {"pair_analyses": [
        _pair([_group([3], [4])]),
    ]}
...
def test_returns_false_when_any_group_has_both_multi_and_equal() -> None:
    # KEY strict-refinement witness vs iter 201: ic = oc = [3, 4].
    # Iter 201 fires (per-group set-equality); this matcher rejects
    # (per-group |ic| > 1).
    patterns = {"pair_analyses": [
...
def test_returns_false_when_one_of_many_groups_violates_identity() -> None:
    # Universal-over-groups semantic: a single non-fixed-point group
    # fails the whole task even if other groups are fixed points.
    patterns = {"pair_analyses": [
        _pair([
            _group([0], [0]),       # fixed point -- ok
            _group([1], [2]),       # non-identity singleton -- offending
            _group([3], [3]),       # fixed point -- ok
        ]),
    ]}
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_one_of_many_groups_violates_cardinality() -> None:
    patterns = {"pair_analyses": [
        _pair([
            _group([0], [0]),       # fixed point -- ok
            _group([1, 2], [1, 2]), # multi-equal -- offending
            _group([3], [3]),       # fixed point -- ok
        ]),
    ]}
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_any_pair_has_offending_group() -> None:
    ...
    patterns = {
        "pair_analyses": [
            _pair([_group([3], [3])]),
            _pair([_group([3], [4])]),  # offending non-fixed-point
            _pair([_group([3], [3])]),
        ],
    }
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_groups_empty_on_any_pair() -> None:
    # Identity-territory rejection (mirroring iter 13 / 215 / 201
...
def test_returns_false_when_any_group_is_not_dict() -> None:
    analysis = _pair([_group([0], [0])])
    analysis["groups"] = [_group([0], [0]), "not-a-dict"]
    patterns = {"pair_analyses": [analysis]}
    assert _matcher()(patterns, {}) is False
...
def test_returns_false_when_color_list_contains_bool() -> None:
    ...
    # (same posture as iter 14 / 18 / 200-206 / 213 / 214 / 215).
...
def test_returned_value_is_boolean_not_truthy() -> None:
    ...
    out_true = _matcher()({"pair_analyses": [_pair([_group([0], [0])])]}, {})
    out_false = _matcher()({"pair_analyses": [_pair([_group([0], [3])])]}, {})
...
def test_ignores_dimensional_fields() -> None:
    ...
    analysis = _pair([_group([0], [0])], input_height=7, input_width=9,
...
def test_strict_refinement_of_iter_215_singleton_recolor_per_group() -> None:
    ...
    # the per-group singletons differ (iter 215 still fires; this
    # matcher rejects).
    ...
    p2 = {"pair_analyses": [_pair([_group([3], [4])])]}
    assert iter215(p2, {}) is True and _matcher()(p2, {}) is False


def test_strict_refinement_of_iter_201_per_group_set_equality() -> None:
    # Strict refinement: this matcher fires => iter 201 fires (per-
    # group set(ic) == set(oc) is a precondition). Converse fails on
    # multi-cardinality set-equality (iter 201 still fires; this
    # matcher rejects).
    iter201 = CONDITION_REGISTRY["output_colors_equals_input_colors_per_group"]

    # This matcher fires => iter 201 fires.
    p1 = {"pair_analyses": [_pair([_group([3], [3])])]}
    assert _matcher()(p1, {}) is True and iter201(p1, {}) is True

    # Iter 201 fires, this matcher rejects: |ic| > 1 set-equal.
...
def test_mutual_exclusion_with_identity_transformation() -> None:
    ...
    # requires >= 1 group). Together they form a complete two-cell
    # identity-rule-family axis (no-blob identity AND per-group
    # singleton-blob identity); the cells are disjoint by the
    # #groups == 0 vs >= 1 split.
...
def test_independent_of_iter_216_singleton_recolor() -> None:
    ...
    # != C). This matcher pins PER-GROUP |ic| == |oc| == 1 AND
    # per-group C_g == K_g (no cross-group identity required).
    ...
    # with C != K (every group recoloured 3 -> 0).
    p1 = {"pair_analyses": [
        _pair([_group([3], [0]), _group([3], [0])]),
    ]}
    assert iter216(p1, {}) is True and _matcher()(p1, {}) is False

    # This matcher fires, iter 216 rejects: per-group fixed points
    # vary across groups (C_g differs).
    p2 = {"pair_analyses": [
        _pair([_group([3], [3]), _group([5], [5])]),
    ]}
...
def test_strict_refinement_of_iter_213_consistent_color_mapping_per_group() -> None:
    ...
    # (e.g. iter 215's "iter 213 alone" witness).
...
def test_strict_refinement_of_iter_197_at_K_prod_eq_1() -> None:
    ...
    # per-group fixed point. Strict refinement of iter 197 at the
    # K_prod == 1 sub-axis.
    ...
    # Iter 197 fires at K_prod == 1 with C != K (per-group bijective
    # singleton non-identity recolour); this matcher rejects.
    p2 = {"pair_analyses": [
        _pair([_group([3], [0])]),
        _pair([_group([5], [1])]),
    ]}
...
def test_strict_implication_of_iter_14_and_iter_18_with_C_eq_K_globally() -> None:
    ...
    # fails when per-group fixed points vary across groups (this
    ...
    # This matcher fires (per-group fixed points), iter 14 rejects
...
def test_independent_of_iter_10_sequential_recoloring() -> None:
    ...
    # contiguous range) is INDEPENDENT of the per-group fixed-point
    # claim. Co-fire / decouple witnesses:
    iter10 = CONDITION_REGISTRY["sequential_recoloring"]

    # Co-fire: per-group fixed points form a contiguous range.
    # ic = oc = [3] / [4] / [5] across three groups (per-pair oc
    # range [3, 4, 5] is contiguous; per-group |ic| == |oc| == 1
    # with ic == oc).
    p1 = {
        "pair_analyses": [
            _pair([_group([3], [3]), _group([4], [4]), _group([5], [5])]),
            _pair([_group([3], [3]), _group([4], [4]), _group([5], [5])]),
        ],
    }
    assert iter10(p1, {}) is True and _matcher()(p1, {}) is True

    # This matcher fires, iter 10 rejects: per-group fixed points
    # do not form a contiguous range (3, 7 -- not contiguous).
    p2 = {"pair_analyses": [
        _pair([_group([3], [3]), _group([7], [7])]),
    ]}
...
def test_recognized_conditions_includes_singleton_recolor_identity_per_group() -> None:
    ...
    assert MATCHER_NAME in fired, (
        f"{MATCHER_NAME!r} did not fire on a per-group singleton-"
        f"fixed-point patterns dict; got {fired!r}"
    )


def test_recognized_conditions_excludes_on_non_identity_singleton() -> None:
    # The iter-10 canonical fixture has per-group ic = [0], oc = [3]
    # (etc.) -- iter 215 fires (both sides singletons), but THIS
    # matcher rejects (ic != oc). Iter 215 must still be in the
    # fired list (no regression).
    from agent.conditions import recognized_conditions
    patterns = {
        "pair_analyses": [
    ...
    assert MATCHER_NAME not in fired, (
        f"{MATCHER_NAME!r} should NOT fire on non-fixed-point singletons; "
        f"got {fired!r}"
    )
    # No regression on iter 215.
    assert "singleton_recolor_per_group" in fired, (
        "iter 215 regression: singleton_recolor_per_group should still "
        f"fire on per-group singletons; got {fired!r}"
    )
