"""
Tests for the static DSL layer (iter 4): the two frozen primitives
`make_grid` / `coloring` and the `apply_DSL` dispatcher (CLAUDE.md §6).
...
Locks:
  ...
  4. Composition: make_grid + coloring reconstructs easy000a's constant output
     grid. This is the exact recipe the next iter's PredictByAllPairCommOp will
...
def test_apply_dsl_dispatch_and_closure():
    assert static_primitives() == ["coloring", "make_grid"]  # F3: frozen at two
...
def test_compose_reconstructs_constant_output():
    """make_grid + coloring rebuilds easy000a's output grid (6x6, (5,5)=2)."""
