"""
tests/test_change_palette_union_count_per_group_constant_across_pairs.py
-- exercise the iter-209 matcher
``agent.conditions.change_palette_union_count_per_group_constant_across_pairs``
(new in this iter).
...
Sits on the per-group union-cardinality sub-axis -- the per-group
projection of iter 45
(``palette_union_count_constant_across_pairs``, whole-grid |A ∪ B|)
and the sibling of iter 207
(``change_palette_intersection_count_per_group_constant_across_pairs``,
per-group |ic ∩ oc|) and iter 208
(``change_palette_symmetric_difference_count_per_group_constant_across_pairs``,
per-group |ic △ oc|) on the same per-group cardinality sub-axis.
Independent of iters 200 / 201 / 202 / 203 / 204 / 205 / 206 / 207 /
208 in general; the per-group cardinality triple {iter 207, iter 208,
this matcher} is closed under any-two-imply-third by the linear
identity |△| = |∪| - |∩|.

Runs without pytest:

    python tests/test_change_palette_union_count_per_group_constant_across_pairs.py
...
def test_returns_true_on_single_pair_single_group_K_equals_two() -> None:
    # |{1, 2} ∪ {1, 2}| == 2 -- per-group equality fires iter 201;
    # this matcher fires with K==2.
...
def test_returns_true_on_single_pair_single_group_K_equals_three() -> None:
    # |{1, 2} ∪ {2, 3}| == 3 (one shared, one each unique).
    patterns = {"pair_analyses": [
...
def test_returns_true_on_two_pairs_K_constant_equals_three() -> None:
    patterns = {"pair_analyses": [
        _analysis(groups=[_group(input_colors=[1, 2], output_colors=[2, 3])]),
...
def test_returns_true_on_multi_group_per_pair_all_K_three() -> None:
    patterns = {"pair_analyses": [
        _analysis(groups=[
...
def test_returns_true_on_three_pairs_K_constant_equals_two() -> None:
    # Per-group equality everywhere -- iter 201 fires, this matcher
    # fires with K==2 (each group's union equals its identical |ic|).
    patterns = {"pair_analyses": [
        _analysis(groups=[_group(input_colors=[1, 2], output_colors=[1, 2])]),
...
def test_returns_true_on_color_zero_in_union() -> None:
    # Colour 0 valid; |{0, 1} ∪ {0, 2}| == 3 across both pairs.
    patterns = {"pair_analyses": [
...
def test_returns_true_on_color_nine_in_union() -> None:
    # Colour 9 valid; |{8, 9} ∪ {7, 9}| == 3 across both pairs.
    patterns = {"pair_analyses": [
...
def test_returns_true_on_varying_num_groups_per_pair_all_K_three() -> None:
    patterns = {"pair_analyses": [
        _analysis(groups=[_group(input_colors=[1, 2], output_colors=[2, 3])]),
        _analysis(groups=[
...
def test_returns_false_when_K_varies_across_pairs() -> None:
    # Pair 0 group: |∪| == 3; pair 1 group: |∪| == 4. Reject.
    patterns = {"pair_analyses": [
        _analysis(groups=[_group(input_colors=[1, 2], output_colors=[2, 3])]),
        _analysis(groups=[_group(
...
def test_returns_false_when_one_group_in_one_pair_breaks_constancy() -> None:
    # Three of four groups have |∪| == 3; fourth has |∪| == 4.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),
            _group(input_colors=[4, 5], output_colors=[5, 6]),
...
def test_returns_false_when_first_group_is_K_two_and_rest_K_three() -> None:
    # Canonical-K-from-first-group: K_canon == 2; subsequent K == 3 must
    # reject.
    patterns = {"pair_analyses": [
        _analysis(groups=[
...
def test_iter201_independent_when_palette_size_varies() -> None:
    # Iter 201 (per-group equality) fires with varying per-group |ic|
    # across pairs (each group is self-equal but sizes differ): iter 201
    # fires; |ic ∪ oc| = |ic| also varies. This matcher REJECTS.
    patterns = {"pair_analyses": [
...
def test_co_fires_with_iter201_when_palette_size_constant() -> None:
    # Iter 201 with constant per-group |ic| == |oc| == 2 across pairs:
    # iter 201 fires, |ic ∪ oc| == 2 on every group, this matcher fires.
    patterns = {"pair_analyses": [
        _analysis(groups=[_group(input_colors=[1, 2], output_colors=[1, 2])]),
        _analysis(groups=[_group(input_colors=[5, 6], output_colors=[5, 6])]),
    ]}
...
def test_independent_of_iter200_when_ic_size_varies() -> None:
    # Per-group oc ⊆ ic with VARYING |ic|: iter 200 fires;
    # |ic ∪ oc| == |ic| varies. This matcher REJECTS.
    patterns = {"pair_analyses": [
        _analysis(groups=[
...
def test_independent_of_iter202_when_oc_size_varies() -> None:
    # Per-group ic ⊆ oc with VARYING |oc|: iter 202 fires;
    # |ic ∪ oc| == |oc| varies. This matcher REJECTS.
    patterns = {"pair_analyses": [
        _analysis(groups=[
...
def test_independent_of_iter206_when_union_size_varies() -> None:
    # Per-group partial-overlap with VARYING |ic ∪ oc|: iter 206 fires
    # (each group is partial-overlap), but the union cardinality varies.
    # group 0: ic={1,2}, oc={2,3}: |∩|=1, |∪|=3.
    # group 1: ic={4,5,6}, oc={5,6,7}: |∩|=2 (NOT partial overlap --
    #     {4} \ {5,6,7} != ∅, {7} \ {4,5,6} != ∅, |∩|>=1, valid partial),
    #     |∪|=4. Different union size.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),     # |∪|=3
            _group(input_colors=[4, 5, 6], output_colors=[5, 6, 7]),  # |∪|=4
        ]),
    ]}
...
def test_co_fires_with_iter206_when_union_size_constant() -> None:
    # Per-group partial-overlap with CONSTANT |ic ∪ oc| == 3 across
...
def test_independent_of_iter207_when_intersection_size_varies() -> None:
    ...
    # |∪| = |ic|+|oc|-|∩|. Hold |∪|=3 with |∩| varying:
    # group 0: ic={1,2}, oc={2,3}: |∩|=1, |∪|=3.
    # group 1: ic={1,2,3}, oc={1,2,3}: |∩|=3, |∪|=3.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),     # |∩|=1, |∪|=3
            _group(input_colors=[1, 2, 3], output_colors=[1, 2, 3]),  # |∩|=3, |∪|=3
        ]),
    ]}
    iter207 = CONDITION_REGISTRY[
        "change_palette_intersection_count_per_group_constant_across_pairs"
    ]
    # |∩| varies (1, 3) ⇒ iter 207 rejects; |∪| == 3 constant ⇒ this fires.
    assert iter207(patterns, {}) is False
    assert _matcher()(patterns, {}) is True


def test_iter207_independent_when_union_size_varies() -> None:
    # Converse witness: |ic ∩ oc| constant but |ic ∪ oc| varies.
    # group 0: ic={1,2}, oc={2,3}: |∩|=1, |∪|=3.
    # group 1: ic={1,2,3}, oc={3,4,5}: |∩|=1, |∪|=5.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),
...
def test_co_fires_with_iter207_when_both_cardinalities_constant() -> None:
    # |ic ∩ oc| == 1 AND |ic ∪ oc| == 3 on every group.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),
            _group(input_colors=[4, 5], output_colors=[5, 6]),
...
def test_independent_of_iter208_when_delta_size_varies() -> None:
    # |ic ∪ oc| constant but |ic △ oc| varies.
    # |△| = |∪|-|∩|. Hold |∪|=3 with |∩| varying ⇒ |△| varies.
    # group 0: ic={1,2}, oc={2,3}: |∩|=1, |∪|=3, |△|=2.
    # group 1: ic={1,2,3}, oc={1,2,3}: |∩|=3, |∪|=3, |△|=0.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),
            _group(input_colors=[1, 2, 3], output_colors=[1, 2, 3]),
        ]),
    ]}
    iter208 = CONDITION_REGISTRY[
        "change_palette_symmetric_difference_count_per_group_constant_across_pairs"
    ]
    # |△| varies (2, 0) ⇒ iter 208 rejects; |∪| == 3 ⇒ this fires.
    assert iter208(patterns, {}) is False
    assert _matcher()(patterns, {}) is True


def test_iter208_independent_when_union_size_varies() -> None:
    # Converse witness: |ic △ oc| constant but |ic ∪ oc| varies.
    # group 0: ic={1,2}, oc={2,3}: |△|=2, |∪|=3.
    # group 1: ic={1,2,3}, oc={2,3,4}: |△|=2, |∪|=4.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),
            _group(input_colors=[1, 2, 3], output_colors=[2, 3, 4]),
        ]),
    ]}
    iter208 = CONDITION_REGISTRY[
        "change_palette_symmetric_difference_count_per_group_constant_across_pairs"
    ]
    assert iter208(patterns, {}) is True
    assert _matcher()(patterns, {}) is False


def test_co_fires_with_iter208_when_both_cardinalities_constant() -> None:
    # |ic △ oc| == 2 AND |ic ∪ oc| == 3 on every group.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),
            _group(input_colors=[4, 5], output_colors=[5, 6]),
...
def test_per_group_cardinality_triple_any_two_imply_third() -> None:
    # The linear identity |△| = |∪| - |∩| on the per-group derived
    # integers means that constancy of any TWO of {|∩|, |△|, |∪|} on
    # a fixture implies constancy of the third on that fixture.
    # Witness: hold |∪|=3 AND |∩|=1 ⇒ |△| forced to 2 on every group.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),       # |∩|=1, |∪|=3, |△|=2
            _group(input_colors=[4, 5], output_colors=[5, 6]),       # |∩|=1, |∪|=3, |△|=2
            _group(input_colors=[7, 8], output_colors=[8, 9]),       # |∩|=1, |∪|=3, |△|=2
        ]),
    ]}
    iter207 = CONDITION_REGISTRY[
        "change_palette_intersection_count_per_group_constant_across_pairs"
...
def test_independent_of_iter45_whole_grid_union() -> None:
    # Whole-grid |A ∪ B| constant across pairs (iter 45) but per-group
    # |ic ∪ oc| varies within a pair: iter 45 fires; this matcher
    ...
    patterns = {"pair_analyses": [
        _analysis(
            ...
            input_palette=[1, 2],
            output_palette=[2, 3, 4, 5],   # whole-grid |∪| = |{1, 2, 3, 4, 5}| = 5
        ),
        _analysis(
            groups=[
                _group(input_colors=[1, 2], output_colors=[2, 3]),
    ...
    iter45 = CONDITION_REGISTRY[
        "palette_union_count_constant_across_pairs"
    ]
    assert iter45(patterns, {}) is True   # whole-grid |∪| == 5 on both
    assert _matcher()(patterns, {}) is False  # per-group |∪| varies (3 vs 5)


def test_independent_of_iter195_when_union_size_varies() -> None:
    # Per-group |ic| constant (iter 195 fires) but |ic ∪ oc| varies.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),     # |ic|=2, |∪|=3
...
def test_co_fires_with_iter195_when_both_axes_constant() -> None:
    # Per-group |ic| == 2 AND per-group |∪| == 3 on every group.
    patterns = {"pair_analyses": [
        _analysis(groups=[
            _group(input_colors=[1, 2], output_colors=[2, 3]),
            _group(input_colors=[4, 5], output_colors=[5, 6]),
...
def test_independent_of_iter14_with_singleton_input_and_mixed_union() -> None:
...
def test_recognized_conditions_includes_matcher_on_constant_K_partial_overlap() -> None:
    ...
    assert MATCHER_NAME in fired, (
        f"{MATCHER_NAME!r} did not fire on per-group constant |∪|==3; "
        f"got {fired!r}"
    )


def test_recognized_conditions_excludes_on_varying_union_size() -> None:
    from agent.conditions import recognized_conditions
    patterns = {
        "pair_analyses": [
            _analysis(groups=[
                _group(input_colors=[1, 2], output_colors=[2, 3]),       # |∪|=3
                _group(input_colors=[4, 5], output_colors=[5, 6, 7, 8]), # |∪|=5
            ]),
        ],
    }
    fired = recognized_conditions(patterns)
...
def test_recognized_conditions_includes_on_per_group_disjoint_K_two() -> None:
    # On the per-group disjoint cell with constant |ic|+|oc|=2 (iter 203
    # fires), this matcher MUST fire with K==2 (|∪|=|ic|+|oc| under
    # disjointness).
    from agent.conditions import recognized_conditions
    patterns = {
        "pair_analyses": [
    ...
    assert MATCHER_NAME in fired, (
        "iter 203 fires AND |ic|+|oc|=2 constant ⇒ this matcher must fire"
    )
