"""
Batch evaluation script — runs all training tasks and reports accuracy.
...
def main():
    parser = argparse.ArgumentParser(description="Batch eval over all training tasks")
    ...
    parser.add_argument("--max-tasks", type=int, default=0,
                        help="Stop after N tasks (0 = all, for quick smoke-tests)")
    ...
    print(f"=== run_1ktasks: {total} tasks from {data_dir} ===")
    print(f"    timeout={args.timeout}s  max_steps={MAX_STEPS}")
    print(f"    started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

    # Load agent once (shares procedural memory across all tasks)
